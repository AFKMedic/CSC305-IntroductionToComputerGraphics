...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    Vector3d tmp_p;
    Vector3d tmp_N;
    int intersection = find_nearest_object(ray_origin, ray_direction, tmp_p, tmp_N);
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        Vector3d bisector = ((ray_origin - p).normalized() + (light_position - p).normalized()).normalized();
        Vector4d specular = spec_color *  std::max(0.0, pow((N.dot(bisector)), obj_specular_exponent));
